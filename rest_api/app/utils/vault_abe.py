...
class Vault():
    ...
    def __init__(self, host: str, username = "itrack_admin", password = "password") -> None:
        ...
        try:
            ...
            logging.error('Unexpected response:'+str(response))
        
    def encrypt_abe(self, data: str, policy_attrs: list[str]) -> str:
        assert self._client_token != None, 'Cannot encrypt without authentication token'
        temp = []
    ...
    def decrypt_abe(self, encrypted_data: str, sub_policy: list[str]) -> str:
        assert self._client_token != None, 'Cannot decrypt without authentication token'
        try:
